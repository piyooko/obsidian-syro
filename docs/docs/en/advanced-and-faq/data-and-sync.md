...
-   _(You may also see temporary files with the `.overlay` suffix. These are incremental cache layers used to avoid write conflicts and speed up updates to large files. They are normal.)_
...
_Tip: before making any manual edits to the underlying JSON files, always copy the folder first as a safety backup._
