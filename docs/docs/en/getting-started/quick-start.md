...
> _Note: This page walks you through the smallest complete Syro workflow. We strongly recommend performing the steps once in your own vault - it is far more effective than reading any long manual._
...
1. **Create a Q/A card**: in a blank area of the note, type a question, separate it with `::`, and then type the answer.
   _Example: `What is Obsidian's core philosophy?:: It is a local-first knowledge base built on plain text.`_
2. **Create a cloze card**: highlight an important phrase in the note using Obsidian's built-in syntax `==text==`.
   _Example: `Syro is a review plugin built on the ==spaced repetition== model.`_
