...
-   _(Click any item in the sidebar to open the original note in the main editor and continue reading.)_
...
_(Important: after you finish reading, remember to perform a `Review` or `Advance` action on the note. This removes it from today's queue and lets the algorithm assign the next revisit date.)_
