/**
 * 这个文件主要是干什么的：
 * 文件系统适配器接口 (ISRFile) 及其实现 (SrTFile)。
 * 它对 Obsidian 的 `TFile` 进行了封装，提供统一的 read/write 接口，并处理 Frontmatter 和 Tag 的读取。
 * 这使得核心逻辑可以与 Obsidian API 解耦（方便测试或迁移）。
 *
 * 它在项目中属于：适配器层 (Adapter Layer)
 *
 * 它会用到哪些文件：
 * 1. Obsidian API
 *
 * 哪些文件会用到它：
 * 1. src/Note.ts (Note 对象持有 SRFile)
 * 2. src/NoteFileLoader.ts (加载文件)
 */
/**
 * [适配器] 包装 TFile，提供读写接口。
 */
