...
export default {
    ...
    "GITHUB_ISSUES_MODIFIED_PLUGIN": 'Raise an <a href="${issuesUrl}">issue</a> if you have a feature request, bug report, or Syro-specific regression.',
    ...
    "FLASHCARD_TAGS_DESC": "注意：通过标签转换卡片已不再推荐。建议使用下方的“文件夹转换为卡组”选项。此处输入的标签仅为兼容旧版使用。",
    "CONVERT_FOLDERS_TO_DECKS": "将文件夹内容转换为卡片组 (推荐)",
    "CONVERT_FOLDERS_TO_DECKS_DESC": "根据文件结构自动生成牌组。这是目前的标准用法，替代了手写标签的方式。",
    ...
    "TAGS_TO_IGNORE": "忽略标签",
    "TAGS_TO_IGNORE_DESC": "输入需要忽略的标签，以空格或换行分隔。带有这些标签的笔记在复习时会被跳过。",
    ...
    "WMS_ALGORITHM_DESC": "专为渐进阅读设计的调度算法，通过重要性权重和间隔继承机制提供灵活的复习节奏",
    ...
    "POST_ISSUE_MODIFIED_PLUGIN": 'Post an <a href="${issue_url}">issue</a> if you find a settings-specific Syro bug.',
    ...
    "SETTINGS_NOTE_CACHE_PERSISTENCE_DESC": "将解析后的笔记保存到 note_cache.json，重启后可复用未变化文件的缓存。",
    ...
    "SETTINGS_CARD_CAPTURE_REBUILD_CONFIRM": "抓卡规则已变化。需要执行一次“重建全部卡片”，重新解析所有笔记，现有笔记才能按新的抓卡规则生成卡片。\n\n是否现在重建全部卡片？",
    "SETTINGS_CARD_CAPTURE_REBUILD_QUEUED": "当前已有同步正在进行，已为你排队“重建全部卡片”。当前同步结束后会自动开始。",
    "SETTINGS_SECTION_SYNC": "同步",
    "SETTINGS_AUTO_INCREMENTAL_SYNC": "自动增量同步",
    "SETTINGS_AUTO_INCREMENTAL_SYNC_DESC": "关闭后，文件变更、进入复习和后台轮询不会自动执行增量同步。你仍可手动执行增量同步，启动时需要的初始化或自动重建会保留。",
    ...
    "SETTINGS_HIDE_FILTER_BAR_DESC": "隐藏筛选栏头部（搜索、排序按钮），仅显示标签。隐藏时强制使用自定义标签排序。",
    "SETTINGS_SHOW_SIDEBAR_PROGRESS_INDICATOR": "显示侧边栏进度指示器",
    "SETTINGS_SHOW_SIDEBAR_PROGRESS_INDICATOR_DESC": "控制是否在复习队列侧边栏的笔记条目上显示进度指示器。",
    "SETTINGS_SIDEBAR_PROGRESS_INDICATOR": "侧边栏进度指示器",
    "SETTINGS_SIDEBAR_PROGRESS_INDICATOR_DESC": "在已显示进度指示器时，选择使用环形进度还是百分比文本。",
    ...
    "SETTINGS_SIDEBAR_PROGRESS_RING_COLOR_DESC": "设置复习队列侧边栏中阅读进度指示器的颜色，环形与百分比文本都会使用它。环形模式下 0% 仍保持空心。",
    "SETTINGS_SIDEBAR_PROGRESS_RING_DIRECTION": "侧边栏进度环方向",
    "SETTINGS_SIDEBAR_PROGRESS_RING_DIRECTION_DESC": "设置进度环从顶部开始向右顺时针增长，还是向左逆时针增长。",
    ...
    "SETTINGS_TIMELINE_ENABLE_DURATION_PREFIX_DESC": "启用 Timeline 中前置时长语法的解析与渲染，例如 2d:: 或 1mo20d::",
    ...
    "SETTINGS_RUNTIME_DEBUG_MESSAGES_DESC": "在开发者控制台显示运行期调试日志，例如同步流程、牌组树刷新和复习会话状态变化。",
    "SETTINGS_ENABLE_CARD_TRACE": "对象级调试追踪 (开发者)",
    "SETTINGS_ENABLE_CARD_TRACE_DESC": "开启后，将在卡片对象本体内收集其从解析提取一直到排期调度的全部生命数据，方便排查数据异常流向。可在卡片界面的「调试与统计」中预览。（推荐仅在找Bug时开启以节省内存）",
    "SETTINGS_SECTION_SUPPORTER": "支持者",
    "SETTINGS_SUPPORTER_DESC_PRO": "持续支持项目长期发展。",
    "SETTINGS_SUPPORTER_DESC_FREE": "Syro 将始终提供 90% 的核心功能。\n激活 License Key 维护插件开发并获得额外功能。",
    ...
    "DECK_OPTIONS_LEARNING_STEPS_DESC": "间隔之间用空格分隔（如 1m 10m）。支持 s(秒) m(分) h(时) d(天)",
    ...
    "DECK_OPTIONS_RELEARNING_STEPS_DESC": "间隔之间用空格分隔（如 10m）。支持 s(秒) m(分) h(时) d(天)",
    ...
    "NOTICE_UPDATE_AVAILABLE": "BRAT Plugins 中有可用的新版 Syro。\n\n你当前使用的是 ${currentVersion}。\n最新版本是 ${latestVersion}。",
