...
const settings_DefaultTagDecks: SRSettings = { ...DEFAULT_SETTINGS };
settings_DefaultTagDecks.convertFoldersToDecks = false;
const parserWithDefaultSettings: NoteQuestionParser =
    createTest_NoteQuestionParser(settings_DefaultTagDecks);
...
A::B
    ...
    // Legacy regression coverage for a previously reported parser edge case.
...
Stop trying ==to milk the crowd== for sympathy. // 写芯懈褌褜 褌芯谢锌褍
`;
            const noteFile: ISRFile = new UnitTestSRFile(noteText);
            ...
            const expected = [
                {
                    ...
                    cards: [
                        {
                            front: "Stop trying ««SR_H:%5B...%5D»» for sympathy. // 写芯懈褌褜 褌芯谢锌褍",
                            back: "Stop trying ««SR_S:to%20milk%20the%20crowd»» for sympathy. // 写芯懈褌褜 褌芯谢锌褍",
                        },
                    ],
                },
            ];
